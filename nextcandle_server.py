...
MIN_QUALITY   = float(os.environ.get('MIN_QUALITY','4.0'))  # corrigé : était 5.5, jamais atteint
...
def get_session():
    ...
    # ⚠ Ordre important : Overlap AVANT London (sinon Overlap est absorbé par 7<=h<17)
    if 13<=h<17:  return{'nm':'Overlap','ov':True}
    if 7<=h<13:   return{'nm':'London','ov':False}
    if 17<=h<22:  return{'nm':'New York','ov':False}
    if 0<=h<7:    return{'nm':'Tokyo','ov':False}
...
def _structure_label(cd):
    if len(cd)<10: return 'NONE'
    sh,sl=detect_swings(cd,3)
    if len(sh)>=2 and len(sl)>=2:
        hh=cd[sh[-1]['i']]['h']>cd[sh[-2]['i']]['h']
        hl=cd[sl[-1]['i']]['l']>cd[sl[-2]['i']]['l']
        lh=cd[sh[-1]['i']]['h']<cd[sh[-2]['i']]['h']
        ll=cd[sl[-1]['i']]['l']<cd[sl[-2]['i']]['l']
        if hh and hl: return 'HH+HL'
        if lh and ll: return 'LH+LL'
        if hh and ll: return 'EXPANSION'
        if lh and hl: return 'CONTRACTION'
    return 'RANGE'
...
def analyze(tf,htf):
    ...
    # ── QUALITY 0-10 (corrigé — l'ancienne formule q/3.0 plafonnait à 2.1) ──
    # confluence : max 4.0 pts · HTF : 0 ou 2.5 · oracle : 0-2.5 · RR : 0-1.0
    confluence_pts = min(4.0, len(levels) * 0.67)
    htf_pts        = 2.5 if htf_dir==setup_dir else 0
    oracle_pts     = max(0.0, min(2.5, (o_prob-55)/45*2.5))
    rr_pts         = (0.5 if rr1>=2.5 else 0)+(0.5 if rr2>=3.5 else 0)
    quality=round(min(10.0,max(0.0, confluence_pts+htf_pts+oracle_pts+rr_pts)),2)
    ...
    return{
        ...
        'sources':levels[:5],
        'structure_label':_structure_label(cd),
    }
...
def record_trade_result(tf, setup, result):
    ...
    _sb_upsert('trading_journal',{
        ...
        'structure_label':setup.get('structure_label','NONE'),
        'ml_regime':setup.get('regime','normal'),
    })
...
def record_oracle_snapshot(closed_tf, close_price):
    ...
    for tf in('m5','m15','h1'):
        ...
        # Corrigé : on enregistre TOUS les TF même si dir=0 ou prob<55
        # L'ancienne condition ignorait M15/H1 hors heures actives
